...
def extract_size_name(item):
    ...
    if re.search(r'\b\d+\s*(2day|2-days|2Day|2Days)\b', text) or '2 days' in text or '2days' in text:
        duration = '2 days'
    elif re.search(r'\b\d+\s*(week|weeks|weekly|Weekly)\b', text) or '7days' in text or '7 days' in text or 'Weekly' in text:
        duration = 'weekly'
    elif re.search(r'\b\d+\s*(month|months)\b',text) or '30days' in text or '30 days' in text or "Monthly" in text or "monthly" in text:
        duration = 'monthly'
    ...
    elif re.search(r'\b\d+\s*(day|days)\b', text) or 'hrs' in text:
        duration = 'daily'
